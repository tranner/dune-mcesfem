...
Ms = set(Ms)
normM = mpl.colors.LogNorm( vmin=min(Ms), vmax=max(Ms) )
smM = pylab.cm.ScalarMappable( norm = normM, cmap = 'viridis' )
cmapM = smM.get_cmap()

hss= set(hss)
normH = mpl.colors.LogNorm( vmin=min(hss), vmax=max(hss) )
smH = pylab.cm.ScalarMappable( norm = normH, cmap = 'viridis_r' )
cmapH = smH.get_cmap()
...
for i, (M, errorPairs) in enumerate(L2ss.iteritems()):
    ...
    for e,h in errorPairs:
        ax.loglog( h, e, 'x', color=cmapM( normM(M) ) )
    ...
    ax.loglog( hsSorted, meansSorted, '-o', label='$M = {0}$'.format(M), color=cmapM( normM(M) ) )

plt.xlabel('$h$')
plt.ylabel('$L^2$ error')

handles, labels = ax.get_legend_handles_labels()
hl = sorted( zip( handles, labels ), key=lambda x: int(x[1].split('=')[1][:-1]) )
h,l = zip(*hl)
...
for i, (M, errorPairs) in enumerate(L2ss.iteritems()):
    ...
    for e,h in errorPairs:
        ax.loglog( [M], [e], 'x', color=cmapH( normH(h) ) )
    ...
    for h, errors in byhdict.iteritems():
        ...
        if h in byhmeans:
            byhmeans[ h ].append( [M,mean] )
        else:
            byhmeans[ h ] = [ [M,mean] ]

for h, l in byhmeans.iteritems():
    lsorted = sorted(l)
    Ms, es = zip(*lsorted)
    ax.loglog( Ms, es, '-o', label='$h = {0}$'.format(h), color=cmapH( normH(h) ) )

handles, labels = ax.get_legend_handles_labels()
hl = sorted( zip( handles, labels ), key=lambda x: -float(x[1].split('=')[1][:-1]) )
h,l = zip(*hl)
ax.legend( h,l, loc='best' )
